...
adonis2(dist_matrix ~ Month + Group + AST + Sex, data = clinical_info, method = "euclidean", by = "margin")

# ======================== Differential Expression Analysis ======================== #
design <- model.matrix(~ Group + Month + Sex, data = clinical_info)
...
for (group in groups) {
  ...
  entrez_ID <- bitr(rnk$Gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Hs.eg.db")
  ...
  topPathways <- rbind(
    ego@result %>% filter(p.adjust < 0.05 & NES > 0) %>% head(10),
    ego@result %>% filter(p.adjust < 0.05 & NES < 0) %>% tail(10)
  ) %>%
